...
class Usuario {
    ...
    */
    public function criar($nome, $email, $telefone, $senha, $aceitouTermos = 0) {
        ...
        try {
            ...
            // Tenta com aceite_termos (nome real no banco de produção)
            $sql = "INSERT INTO usuarios (nome, email, telefone, senha, aceite_termos)
                    VALUES (:nome, :email, :telefone, :senha, :aceite_termos)";
            ...
            $stmt->bindParam(":aceite_termos", $aceitouTermos, PDO::PARAM_INT);

            return $stmt->execute();

        } catch (PDOException $e) {
            // Fallback: tenta com aceitou_termos (nome alternativo)
            try {
                $senhaHash = password_hash($senha, PASSWORD_DEFAULT);
                $sql2 = "INSERT INTO usuarios (nome, email, telefone, senha, aceitou_termos)
                ...
                $stmt2->bindParam(":senha",         $senhaHash);
                $stmt2->bindParam(":aceitou_termos", $aceitouTermos, PDO::PARAM_INT);
                return $stmt2->execute();
            } catch (PDOException $e2) {
                error_log("Erro ao criar usuário: " . $e2->getMessage());
                return false;
            }
        }
    ...
    */
    public function aceitarTermos($usuario_id) {
        ...
        // Atualiza ambas as colunas para máxima compatibilidade
        try {
            $sql = "UPDATE usuarios SET aceite_termos = 1, aceitou_termos = 1 WHERE id = :id";
            $stmt = $this->conexao->prepare($sql);
            $stmt->bindParam(":id", $usuario_id, PDO::PARAM_INT);
            return $stmt->execute();
        } catch (PDOException $e) {
            // Tenta somente com a coluna disponível
            try {
                $sql2 = "UPDATE usuarios SET aceite_termos = 1 WHERE id = :id";
                $stmt2 = $this->conexao->prepare($sql2);
                $stmt2->bindParam(":id", $usuario_id, PDO::PARAM_INT);
                return $stmt2->execute();
            } catch (PDOException $e2) {
                error_log("Erro aceitarTermos: " . $e2->getMessage());
                return false;
            }
        }
